...
//------------------------------------------------------------------
Ray_VS_Out Ray_VertexShader(float3  pos_		: POSITION)
{
	Ray_VS_Out output = (Ray_VS_Out)0;
	float4  posW = mul(float4(pos_.xyz, 1), mat.world);
	float4  posV = mul(posW, mat.view);
	float4  posP = mul(posV, mat.projection);
...
//------------------------------------------------------------------
float4 Ray_PixelShader(Ray_VS_Out  ip_) : SV_Target
{
	return  mate.objectColor;
}
//------------------------------------------------------------------
...
technique10 MyStdShaderTec
{
	...
	pass RayTest
	{
		SetVertexShader(CompileShader(vs_4_0, Ray_VertexShader()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_4_0, Ray_PixelShader()));
	}
	//☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆
